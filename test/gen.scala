...
generate {
  ...
  on SERVICE { service =>
    source(s"./dsl-output/${service.fullPathDirectory}", s"${service.name.toPascal}Service.java") {
      ...
      //     comment(f)
      //     ln(s"""${f.functionTypeName} ${f.name.toCamel}(${f.params}) " posted successfully!" """)
      //   }
      // }
      ln(s"import static spark.Spark.*;")

      block(s"public class ${service.name.toPascal}") {
        block(s"""public static void main(String[] args)"""){
          ln(s"""post("/articles", (req, res) -> { return "posted successfully!"; });""")
          service.functions.foreach {f =>
            ln(s"""${f.name.toCamel}("${service.serviceUrl}/:${f.argParamName}", (req, res) -> { return req.params(":${f.argParamName}") + " posted successfully!"; });""")
          }
        }
      }
