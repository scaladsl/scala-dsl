...
generate {
  ...
  begin SERVICE { service =>
    file(s"iunetworks/spark/${service.name.toPascal}Api.java") {
      ...
      block(s"public final class ${service.name.toPascal}Api extends BasicApi") {
          ...
          service.functions.foreach { f =>
            ...
            block(s"try"){
              if(ftype(f) != "void"){
                f.modifier match {
                  case "required" =>
                    ln(s"return toJson(new ${service.name.toPascal}ServiceImpl().${f.name.toCamel}(${functionParams(f)}));")
                  case "repeated" => {
                    ln(s"PageInfo paging = pageInfo(req);")
                    ln(s"OrderInfo ordering = orderInfo(req);")
                    ln(s"return toJson(new ${service.name.toPascal}ServiceImpl().${f.name.toCamel}(paging, ordering));")
                  }
                  case _ => ""
                }
              }
